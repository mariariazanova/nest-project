...
export class ConsulService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(private config: ConfigService) {
    ...
    this.serviceId = `history-service-${process.env.HOSTNAME || 'local'}-${Date.now()}`;
  }
  ...
  async registerService() {
    try {
      ...
      const servicePort = parseInt(this.config.get('PORT', '3003'));
      const serviceHost = process.env.SERVICE_HOST || 'history-service';

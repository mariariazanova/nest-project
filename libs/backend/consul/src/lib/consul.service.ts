...
export class ConsulService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    this.serviceId = `${this.options.serviceName}-${process.env['HOSTNAME'] || 'local'}-${Date.now()}`;
  }
  ...
  async registerService() {
    try {
      const { serviceName, tags } = this.options;
      const servicePort = parseInt(this.config.get('PORT', String(this.options.servicePort)));
      const serviceHost = process.env['SERVICE_HOST'] || serviceName;
      ...
      await this.consul.agent.service.register({
        ...
        tags: tags ?? [serviceName, 'microservice', 'nestjs'],
      });
